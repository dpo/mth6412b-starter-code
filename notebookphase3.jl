...
6. Le test unitaire qui vérifiait la longueur de l'arbre de coût minimal (maintenant de longeur fixe) de Kruskal() vérifie maintenant qu'aucun élément "nothing" se trouve dans l'arbre.
7. Dans tous les fichiers où c'est possible, la synthaxe "variable.attribut" a été remplacée par la synthaxe "attribut(variable)".
8. La fonction "test\_kruskal" affiche maintenant le coût total de l'arbre de recouvrement trouvé.
...
11. La fonction "test\_kruskal" comprend un nouvel argument "kruskal\_func" qui permet de tester différentes implémentations de l'algorithme de Kruskal.
"""
#hidden line
...
# ╔═╡ 94ba2fbc-31e8-4a0f-9f7a-a6add3e7bae4
md"""
### Preuve que rang(sommet) ≤ |S|-1

Soit un arbre de |S| sommets. Le rang le plus élevé de ces sommets est celui de la racine r. L'arrangement de ces sommets pour lequel le rang de la racine est le plus grand est celui où tout sommet n'a qu'un enfant, sauf la feuille. Dans ce cas, rang(r) = |S| - 1. Il en suit que tout sommet a un rang ≤ |S| - 1.

### Preuve que rang(sommet) ≤ ⌊log₂(|S|)⌋ pour la procédure d'union par le rang

Par récurrence sur le rang de la racine n (qui est la hauteur).
On va montrer que 2ⁿ ≤ |Sₙ| , où |Sₙ| peut être n'importe quel arbre de rang n créé avec la procédure d'union par le rang.

Pour n = 0, l'ensemble des sommets de l'arbre est un singleton, et 2⁰ = 1 donc la propriété est vérifiée au rang 0.

Au rang n ≥ 0, on suppose la propriété vraie.
Si le rang de la racine vaut n + 1 pour le graphe |Sₙ₊₁|, on sait que dans la procédure le graphe a été créé par l'union de deux graphes de rang au plus n.
On a alors
|Sₙ₊₁| ≥ 2 |Sₙ| ≥ 2ⁿ⁺¹ par hypothèse de récurrence.

Donc la propriété est vraie pour tout n ≥ 0.

On prend le log2 de la propriété au rang n: n ≤ log2(|Sₙ|).

Mais comme n est entier, on peut écrire: n ≤ ⌊log2(|Sₙ|)⌋.

"""

# ╔═╡ 935d5990-86ef-4f8b-9afe-9ca433dfddca
md"""
# 4. Implémentation de la compression des chemins

Le fonction "compressionChemins!()" prend en argument une composante connexe et un sommet à partir duquel effectuer la compression. Comme cette fonction sera utilisée par la fonction "unionRang!()" qui trouve déjà la racine de la composante connexe, nous la passons également en argument. La fonction "compressionChemins!()" n'a alors pas besoin de trouver la racine et est plus rapide. La fonction ne renvoie rien car elle agit directement sur les sommets de la composante connexe. Dans le fichier "mainphase3.jl" nous avons créé une composante connexe sur lesquelle faire des tests. Nous avons également créé une fonction "test_compression()" qui prend en argument une composante connexe, le sommet sur lequel lancer la recherche, et un vecteur solution. La solution permet de vérifier que chaque sommet comporte exactement le bon parent après la compression.
"""

# ╔═╡ 4b4b562d-28be-4ff7-8f8e-856eba045589
begin
	with_terminal() do
...
# ╔═╡ faeb830f-9554-4fec-9e3b-b54d56ae9507
md"""
# 5. Amélioration de l'implémentation de l'algorithme de Kurskal avec l'union par le rang

Nous avons créé une nouvelle fonction se nommant "kruskal\_acc()". Il s'agit d'un copier coller de la fonction de la phase 2, à l'exception de l'union des composantes connexes qui est effectuée par la fonciton "unionRang()", qui elle à son tour utilise la fonction "compressionChemins()". Un simple test qui consiste à ajouter un compteur dans la fonction "unionRang()" a permis de montrer que le nombre total d'itérations est diminué sur tous les graphes des fichiers ".tsp" lorsque la compression des chemins est effectuée. La section 9 présente un banc d'essais comparant les différents algorithmes.
"""

# ╔═╡ 0bddd61c-1feb-44ce-9867-9ece823bab02
md"""
...
# ╔═╡ 9252a29d-93d5-41a9-a7f8-593822a661bd
begin
	with_terminal() do
...
# ╔═╡ 9e297aa2-fc9f-4ec2-aaa1-54b7618130a3
begin
	with_terminal() do		
...
La fonction "test\_prim_all()" performe l'algorithme sur tous les fichiers ".tsp". On observe entre autres que le coût total de bayg29.tsp est de 1319, ce qui est optimal au vu des commentaires sur notre dernier rapport.
"""
#hidden line
...
# ╔═╡ b9ae32b1-0a67-46ad-8a5b-0500dd0ab29d
md"""
